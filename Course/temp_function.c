...
{
    /*В данной реализации список увеличивается влево (стек - LIFO), то есть новые
     * элементы добавляются в начало списка (слева), а голова списка (*head) всегда
     * указывает на самый новый элемент.*/
    ...
    // Записываем переданные данные в новый узел
    new_node->data = data;
    // Устанавливаем указатель next нового узла на текущую голову списка (head).
    new_node->next = *head; // next нового узла указывает на старую вершину (head)
    // Установка указателя prev нового узла в NULL (так как он становится первым)
    new_node->prev = NULL;

    // Если список не был пустым
    if (*head != NULL)
    {
        // Устанавливаем указатель prev старой головы на новый узел
        (*head)->prev = new_node;
    }
...
sensor pop(node** head, node** tail)
{
    ...
    // Сохранение ссылки на текущую голову списка во временной переменной
    node* temp = *head;
    // Сохранение данных из головного узла перед его удалением
    sensor popped_data = temp->data;

    // Перемещение указателя head на следующий узел
    *head = (*head)->next;

    // Проверка, не стал ли список пустым после извлечения
    if (*head != NULL)
    {
        // Если список не пуст, обнуляем указатель prev новой головы
        (*head)->prev = NULL;
    }
    else
    {
        // Если список стал пустым, обнуляем указатель tail
        *tail = NULL;
    }

    // Возвращаем значение из temp и освобождаем память.
    free(temp);
    return popped_data;
}
